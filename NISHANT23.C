   ...
   for (i = 0; i < count; ++i)
		{
		 if (REMAIN[i]==0)
		{
			TA[i]=COMPLETE[i]-ARRIVAL[i];       // calculation of turn around time by process=completion time-arrival time........
			W[i]=TA[i]-BURST[i];                //calculation of waiting time by process=turnaround-worst....... 
			if (W[i]<=0)
			{
				W[i]=0;
			}
